...
export function MarkLayer({ tool, reservedRight, registerCommit }: MarkLayerProps): JSX.Element {
  ...
  const marks = useMemo<Mark[]>(() => {
    ...
    for (const entry of all) {
      ...
      /*
       * Отбора по типу записи здесь НЕТ, и это правило, а не упущение.
       *
       * Пометка — одно понятие на все три инструмента: под кнопкой «Вид»
       * рецензент ждёт одного обращения со всей своей работой. Прежде
       * подчёркивания правок текста подчинялись отдельному тумблеру, а рамка
       * области и точка указателя были видны всегда, — и разделение это шло
       * от того, меняет ли запись DOM. Это факт про реализацию, и в интерфейс
       * ему протекать незачем: рецензент не различает, что виджет умеет
       * наложить, а что только записал (`docs/adr/0001`).
       *
       * Единственное правило видимости меток — вид страницы, и оно уже
       * применено выше одной проверкой на весь набор.
       */
