 ...
 * поднят. Второе правило, пунктир вокруг изменённых мест, снято: изменённые
 * места помечает подчёркиванием теневой слой меток (см. ниже).
 ...
 * каждого применённого места по отдельному тумблеру. Теперь то же самое
 * множество помечает слой меток — корректорским подчёркиванием под правкой
 * текста (`widgets/mark-layer`).
 *
 * Довод от переезда только усилился: тумблера, показывавшего изменённые места
 * по отдельному правилу, больше нет вовсе — подчёркивание видно всегда, когда
 * виден слой правок (FR-22, `docs/adr/0001`). Значит вернуть обводку в чужой
 * `<head>` тем более незачем: она рисовала бы то же самое место вторым
 * способом и в то же самое время.
 *
 ...
 *
 * Сплошная линия против подчёркивания правки: «вот сюда я прокрутил» и «здесь
 * есть правка» не имеют права выглядеть одинаково — иначе на странице
 * с десятком правок цель теряется среди них. Пунктира, с которым это правило
 * различалось прежде, больше нет ни в чужом `<head>`, ни где-либо ещё, — но
 * различать теперь надо с подчёркиванием теневого слоя, и оно видно ВСЕГДА,
 * а не по тумблеру. То есть довод стал строже, а не слабее.
 *
 ...
 * подсветки изменений здесь больше нет, и второго аргумента у функции поэтому
 * тоже: изменённые места помечает теневой слой меток, а не правило в чужом
 * `<head>` (см. выше).
 */
...
export function applyLayer(root: Document = document): Summary {
  ...
  try {
    ...
    // подсветки цели прокрутки (FR-31), нужное и в виде «оригинал» — именно
    // там человек ищет уехавшее место руками. Второго правила здесь нет,
    // и флага, которым оно управлялось, тоже: вид страницы решает судьбу меток
    // целиком и в одном месте — в слое меток (`widgets/mark-layer`).
    syncStyle(root, true)
  } finally {
    watcher?.resume()
  }
