...
describe('buildReport', () => {
  it('вид правки назван ИНСТРУМЕНТОМ, которым она сделана', () => {
    ...
    //
    // У замечаний в той же строке стоит третьим ответом дорожка: место правки
    // словами. У правки текста и пожелания её нет — там место называет «было».
  ...
  it('правки с разных страниц стоят одной описью, и каждая называет свою', () => {
    ...
    expect([...doc.querySelectorAll('.edit__head')].map((el) => el.textContent)).toEqual([
      'Область · /about · main > p',
      'Область · /price · main > p',
      'Область · /about · main > p',
    ])
  })

  it('место замечания называет ДОРОЖКА, а не текст якоря', () => {
    /*
     * Текст якоря у замечания — это `textContent` элемента целиком, то есть
     * обычно весь обведённый блок. В описи он читался простынёй, к самому
     * замечанию относящейся слабо. Место называет дорожка, и стоит она третьим
     * ответом в служебной строке — там же, где инструмент и маршрут.
     */
    const html = buildReport(
      inputOf([
        entryOf('c1', 'comment', {
    ...
    expect(row?.querySelector('.edit__head')?.textContent).toBe(
      'Область · / · Продукция → карточка 2 → заголовок',
    )
    // Серой строки у замечания больше нет вовсе — не пустая, а отсутствующая.
    expect(row?.querySelector('.edit__was')).toBeNull()
    expect(html).not.toContain('все три абзаца подряд')
    // А написанное рецензентом на месте: убирали место, а не правку.
    expect(row?.querySelector('.edit__now')?.textContent).toBe(
      'Этот блок убрать, он дублирует шапку',
    )
  })

  it('у правки текста «было» УЦЕЛЕЛО и дорожка не приписана', () => {
    // Проверяется не только то, что изменилось у замечания, но и то, что
    // осталось у соседа: правило идёт по типу записи, а не по инструменту.
    const row = parse(
      buildReport(inputOf([entryOf('t1', 'text-override', { path: 'Главная → текст' })])),
    ).querySelector('.row')

    expect(row?.querySelector('.edit__was')?.textContent).toBe('Было так')
    expect(row?.querySelector('.edit__head')?.textContent).toBe('Текст · /')
  })

  it('пустая дорожка не оставляет висящего разделителя', () => {
    // Запись из файла, снятого до вехи «Машиночитаемость»: `path` пуст.
    // Строка обязана закончиться маршрутом, а не точкой в пустоту.
    const row = parse(
      buildReport(inputOf([entryOf('c1', 'comment', { route: '/catalog', path: '' })])),
    ).querySelector('.row')

    expect(row?.querySelector('.edit__head')?.textContent).toBe('Область · /catalog')
  })
