...
import { elementAtPoint, setPageCursor, watchPointPicking } from 'shared/lib/dom'
 ...
 *
 * Пауза и курсор устроены как у «Области», и по тем же причинам — разбор там,
 * в шапке `useDrawArea`. Здесь пауза означает «точку не ставим»: клик по
 * странице при открытом окне замечания по-прежнему проглатывается.
 */
export function usePlacePoint(active: boolean): PlacePointTool {
  const [draft, setDraft] = useState<Entry | null>(null)

  /** Черновик для предиката паузы — ссылкой: см. шапку `useDrawArea`. */
  ...
  useEffect(() => {
    ...
    return watchPointPicking(
      ...
      // Пауза: окно замечания открыто — клик глотаем, точку не ставим.
      () => draftRef.current !== null,
    )
  }, [active])

  /* Курсор режима. Условие своё, отличное от перехвата: см. `useDrawArea`. */
